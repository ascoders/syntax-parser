...
export class Tokenizer {
         ...
         private createStringPattern(stringTypes: string[]) {
           const patterns: { [x: string]: string } = { '``': '((`[^`]*($|`))+)', '[]': '((\\[[^\\]]*($|\\]))(\\][^\\]]*($|\\]))*)', '""': '(("[^"\\\\]*(?:\\\\.[^"\\\\]*)*("|$))+)', "''": "(('[^'\\\\]*(?:\\\\.[^'\\\\]*)*('|$))+)", "N''": "((N'[^N'\\\\]*(?:\\\\.[^N'\\\\]*)*('|$))+)" };
         ...
         private getPlaceholderTokenWithKey({ input, regex, parseKey }: { input: string; regex: boolean | RegExp; parseKey: (key: string) => string }) {
         ...
         private getTokenOnFirstAndSecondMatch({ input, type, regex }: { input: string; type: string; regex: boolean | RegExp }) {
           if (typeof regex === 'boolean') {
             return null;
           } else {
             const matches = input.match(regex);
